/**
 * Fabulousa Studio's — REST API Server
 * Ista: Baron Von Hilton (The Styleista)
 * Pipeline Stage: 2 — Visual Logic
 */
import express, { Request, Response, NextFunction } from 'express';
import cors from 'cors';
import helmet from 'helmet';
import morgan from 'morgan';
import { FabricEngine } from '../tailoring/fabric-engine';
import { HexValidator } from '../color/hex-validator';
import { GenerativeCouture } from '../couture/generative-couture';
import { logger } from '../utils/logger';

import { createHash, createHmac } from 'crypto';
const IAM_JWT_SECRET = process.env.IAM_JWT_SECRET || process.env.JWT_SECRET || '';
const SERVICE_ID = 'fabulousa';
const MESH_ADDRESS = process.env.MESH_ADDRESS || 'fabulousa.agent.local';
function sha512Audit(data: string): string { return createHash('sha512').update(data).digest('hex'); }
function b64urlDecode(s: string): string {
  const b64 = s.replace(/-/g, '+').replace(/_/g, '/');
  return Buffer.from(b64 + '='.repeat((4 - b64.length % 4) % 4), 'base64').toString('utf8');
}
interface JWTClaims { sub: string; email?: string; role?: string; active_role_level?: number; permissions?: string[]; exp?: number; jti?: string; }
function verifyIAMToken(token: string): JWTClaims | null {
  try {
    const parts = token.split('.'); if (parts.length !== 3) return null;
    const [h, p, sig] = parts;
    const header = JSON.parse(b64urlDecode(h));
    const alg = header.alg === 'HS512' ? 'sha512' : 'sha256';
    const expected = createHmac(alg, IAM_JWT_SECRET).update(`${h}.${p}`).digest('base64url');
    if (sig !== expected) return null;
    const claims: JWTClaims = JSON.parse(b64urlDecode(p));
    if (claims.exp && claims.exp * 1000 < Date.now()) return null;
    return claims;
  } catch { return null; }
}
function iamRequestMiddleware(req: Request, _res: Response, next: NextFunction): void {
  const traceId = (req.headers['x-trace-id'] as string) || `trc-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  (req as any).traceId = traceId; (req as any).serviceId = SERVICE_ID;
  const authHeader = req.headers.authorization;
  if (authHeader?.startsWith('Bearer ')) {
    const claims = verifyIAMToken(authHeader.slice(7));
    if (claims) { (req as any).user = claims; (req as any).auditHash = sha512Audit(JSON.stringify({ sub: claims.sub, path: req.path, ts: Date.now() })); }
  }
  next();
}

export const fabricEngine = new FabricEngine();
export const hexValidator = new HexValidator();
export const couture = new GenerativeCouture();
export const app = express();
app.use(helmet()); app.use(cors()); app.use(morgan('short'));
app.use(express.json({ limit: '5mb' })); app.use(iamRequestMiddleware);

app.get('/health', (_req, res) => {
  res.json({ status: 'healthy', service: SERVICE_ID, ista: 'Baron Von Hilton — The Styleista', mesh: MESH_ADDRESS, uptime: process.uptime(), timestamp: new Date().toISOString() });
});

// Fabric endpoints
app.post('/fabrics', (req, res) => { try { res.status(201).json(fabricEngine.createFabric(req.body)); } catch (e: any) { res.status(400).json({ error: e.message }); } });
app.get('/fabrics', (_req, res) => { res.json(fabricEngine.getAllFabrics()); });
app.get('/fabrics/:id', (req, res) => { const f = fabricEngine.getFabric(req.params.id); if (!f) return res.status(404).json({ error: 'Not found' }); res.json(f); });
app.post('/fabrics/compose', (req, res) => { const c = fabricEngine.createComposition(req.body); if (!c) return res.status(400).json({ error: 'Invalid composition' }); res.status(201).json(c); });

// Color validation endpoints
app.post('/colors/validate', (req, res) => { res.json(hexValidator.validateHex(req.body.hex, req.body.bgHex)); });
app.post('/colors/palette', (req, res) => { res.json(hexValidator.validatePalette(req.body.colors)); });

// Couture endpoints
app.post('/couture/garments', (req, res) => { try { res.status(201).json(couture.generateGarment(req.body)); } catch (e: any) { res.status(400).json({ error: e.message }); } });
app.get('/couture/garments', (_req, res) => { res.json(couture.getAllGarments()); });
app.get('/couture/garments/:id', (req, res) => { const g = couture.getGarment(req.params.id); if (!g) return res.status(404).json({ error: 'Not found' }); res.json(g); });
app.post('/couture/lines', (req, res) => { try { res.status(201).json(couture.createFashionLine(req.body)); } catch (e: any) { res.status(400).json({ error: e.message }); } });
app.get('/couture/lines', (_req, res) => { res.json(couture.getAllLines()); });

app.get('/dashboard', (_req, res) => {
  res.json({
    service: SERVICE_ID, ista: 'Baron Von Hilton — The Styleista', role: 'Generative Fashion & Style Engine',
    pipeline: 'Stage 2 — Visual Logic',
    fabrics: fabricEngine.getStats(), colors: hexValidator.getStats(), couture: couture.getStats(),
    timestamp: new Date().toISOString(),
  });
});


// ═══════════════════════════════════════════════════════════════════════════════
// 2060 SMART RESILIENCE LAYER — Auto-wired by Trancendos Compliance Engine
// ═══════════════════════════════════════════════════════════════════════════════
import {
  SmartTelemetry,
  SmartEventBus,
  SmartCircuitBreaker,
  telemetryMiddleware,
  adaptiveRateLimitMiddleware,
  createHealthEndpoint,
  setupGracefulShutdown,
} from '../middleware/resilience-layer';

// Initialize 2060 singletons
const telemetry2060 = SmartTelemetry.getInstance();
const eventBus2060 = SmartEventBus.getInstance();
const circuitBreaker2060 = new SmartCircuitBreaker(`${SERVICE_ID}-primary`, {
  failureThreshold: 5,
  resetTimeoutMs: 30000,
  halfOpenMaxAttempts: 3,
});

// Wire telemetry middleware (request tracking + trace propagation)
app.use(telemetryMiddleware);

// Wire adaptive rate limiting (IAM-level aware)
app.use(adaptiveRateLimitMiddleware);

// 2060 Enhanced health endpoint with resilience status
app.get('/health/2060', createHealthEndpoint({
  serviceName: SERVICE_ID,
  meshAddress: MESH_ADDRESS,
  getCustomHealth: () => ({
    circuitBreaker: circuitBreaker2060.getState(),
    eventBusListeners: eventBus2060.listenerCount(),
    telemetryMetrics: telemetry2060.getMetricNames().length,
  }),
}));

// Prometheus text format metrics export
app.get('/metrics/prometheus', (_req: any, res: any) => {
  res.setHeader('Content-Type', 'text/plain; version=0.0.4; charset=utf-8');
  res.send(telemetry2060.exportPrometheus());
});

// Emit service lifecycle events
eventBus2060.emit('service.2060.wired', {
  serviceId: SERVICE_ID,
  meshAddress: MESH_ADDRESS,
  timestamp: new Date().toISOString(),
  features: ['telemetry', 'rate-limiting', 'circuit-breaker', 'event-bus', 'prometheus-export'],
});

// ═══════════════════════════════════════════════════════════════════════════════
// END 2060 SMART RESILIENCE LAYER
// ═══════════════════════════════════════════════════════════════════════════════

app.use((err: Error, _req: Request, res: Response, _next: NextFunction) => {
  logger.error({ err: err.message }, 'Fabulousa: Error — Baron Von Hilton is appalled');
  res.status(500).json({ error: 'Internal server error', service: SERVICE_ID });
});
